...
mbo = function(fun, par.set, des=NULL, learner, control, show.info=TRUE) {
  ...
  if (class(learner)[1]=="list" && is.null(ctrl$ensemble.select))
    stop("For more than one, ensemble.select has to be set")  
  if (class(learner)[1]=="list")
    learners = learner
  ...
  list(x=x, y=y, path=opt.path, resample=res.vals, models=models, sel.learners=sel.learners)
}
