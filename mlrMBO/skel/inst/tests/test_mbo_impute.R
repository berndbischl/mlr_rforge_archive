...
test_that("mbo works with failures", {
  ...
  ctrl = makeMBOControl(n.iters=20, seq.design.points=500)
  expect_error(mbo(f1, ps, des=NULL, learner, ctrl, show.info=FALSE), "Infeasible y")
  ctrl = makeMBOControl(n.iters=20, seq.design.points=500, impute=function(x, y, opt.path) 0)
  mbo(f1, ps, des=NULL, learner, ctrl, show.info=FALSE)
  ctrl = makeMBOControl(n.iters=50, seq.design.points=500)
  expect_error(mbo(f2, ps, des=NULL, learner, ctrl, show.info=FALSE), "foo")
  ctrl = makeMBOControl(n.iters=50, seq.design.points=500, impute=function(x, y, opt.path) 0, impute.errors=TRUE)
