tune.cmaes = function(learner, task, resampling, measure, bounds, control, logger) {
  ...
  g = make.tune.f(ns, penv, learner, task, resampling, measure, control, logger)

  g2 = function(p) {
    p2 = as.list(as.data.frame(p))
    p2 = lapply(p2, function(x) {x=as.list(x);names(x)=ns;x})
    es = eval.states.tune(learner, task, resampling, measure, control, p2, "optim")
