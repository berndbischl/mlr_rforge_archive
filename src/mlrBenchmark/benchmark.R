...
#' @return \code{\linkS4class{BenchResult}}.
#'
...
benchmark <- function(learners, tasks, resamplings, measures, models=FALSE, same.resampling.instance=TRUE)  {
  ...
  if (same.resampling.instance)
    resamplings = Map(function(res,tt) 
        if (is(res, "ResampleInstance")) res  else makeResampleInstance(res,task=tt), resamplings, tasks)
  ...
  return(new("BenchResult", task.descs=tds, learners=learners, resamplings=resamplings,
