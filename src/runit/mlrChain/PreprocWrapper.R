testPreprocStep = function() {
  ...
  checkTrue(setequal(getHyperPars(w2), list(minsplit=10, x=1, y=2))) 
  checkTrue(setequal(getHyperPars(w2, "train"), list(minsplit=10, x=1, y=2))) 
  checkTrue(setequal(w2@par.vals, list(x=1, y=2))) 
  
  wl3 = setHyperPars(w2, minsplit=77, x=88)
  checkTrue(setequal(getHyperPars(wl3), list(minsplit=77, x=88, y=2))) 
  checkTrue(setequal(wl3@par.vals, list(x=88, y=2))) 
  
  m = train(w2, task=multiclass.task)
  print(m)
  checkTrue(setequal(getHyperPars(m@learner), list(minsplit=10, x=1, y=2))) 
} 
