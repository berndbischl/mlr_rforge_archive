...
test.nnet <- function() {
  library(nnet)
	...
	simple.test("classif.nnet", multiclass.df, multiclass.target, multiclass.train.inds, p, parset=list(size=7))
	prob.test("classif.nnet", multiclass.df, multiclass.target, multiclass.train.inds, p2, parset=list(size=7))
	prob.test("classif.nnet", binaryclass.df, binaryclass.target, binaryclass.train.inds, p3, parset=list(size=7))	
	...
	cv.test("classif.nnet", multiclass.df, multiclass.target, tune.train=tt, tune.predict=tp, parset=list(size=3, maxit=50))
