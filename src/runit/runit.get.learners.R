test.get.learners <- function() {
	...
	num1 <- get.learners(type = "classif", numerics = TRUE)
	for(i in seq(length=length(num1))){
		checkTrue(make.learner(num1[i])["numerics"])
	}	
	num2 <- get.learners(type = "regr", numerics = TRUE)
	for(i in seq(length=length(num2))){
		checkTrue(make.learner(num2[i])["numerics"])
	}
...
#	char1 <- get.learners(type = "classif", characters = TRUE)
#	for(i in seq(length=length(char1))){
#		checkTrue(make.learner(char1[i])["characters"])
#	}	
#	char2 <- get.learners(type = "regr", characters = TRUE)
#	for(i in seq(length=length(char2))){
#		checkTrue(make.learner(char2[i])["characters"])
#	}
