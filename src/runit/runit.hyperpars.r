...
test.hyperpars <- function() {
	...
	checkTrue(setequal(wl2["par.vals", par.top.wrapper.only=T], list(x=1, y=2))) 
	checkTrue(setequal(wl2["par.vals.name", par.top.wrapper.only=T], c("x", "y"))) 
	...
	checkTrue(setequal(wl3["par.vals", par.top.wrapper.only=T], list(x=88, y=2))) 
	checkTrue(setequal(wl3["par.vals.name", par.top.wrapper.only=T], c("x", "y"))) 
