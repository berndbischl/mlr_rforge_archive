test.losses <- function() {
	...
	checkEquals(perf3$measures["sse"], sum(perf3$losses[, "squared"]), checkNames=F)
	checkEquals(perf3$measures["sae"], sum(perf3$losses[, "abs"]), checkNames=F)
	checkEquals(perf3$measures["mse"], mean(perf3$losses[, "squared"]), checkNames=F)
	checkEquals(perf3$measures["mae"], mean(perf3$losses[, "abs"]), checkNames=F)
	checkEquals(perf3$measures["medse"], median(perf3$losses[, "squared"]), checkNames=F)
	checkEquals(perf3$measures["medae"], median(perf3$losses[, "abs"]), checkNames=F)
	checkEquals(perf3$losses[, "abs"], abs(perf3$losses[, "residual"]), checkNames=F)
