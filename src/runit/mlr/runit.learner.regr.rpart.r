...
test.rpart.regr <- function() {
  ...
  simple.test.parsets("regr.rpart", regr.df, regr.target, regr.train.inds, old.predicts.list, parset.list)
  
  tt <- "rpart"
  tp <- function(model, newdata) predict(model, newdata)
  
  cv.test.parsets("regr.rpart", regr.df, regr.target, tune.train=tt, tune.predict=tp, parset.list=parset.list)
