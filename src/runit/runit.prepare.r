...
test.prepare <- function(){ 
		data = test.dataset
		desc1 <- make.data.desc(data = test.dataset, target.col = "ints")
		desc2 <- make.data.desc(data = test.dataset, target.col = "chars")

		ints2nums <- prep.data(data = test.dataset, target.col = "ints", data.desc = desc1, 
...
#		checkTrue(is.factor(ints2factors[,1]))
		...
		chars2factors <- prep.data(data = test.dataset, target.col = "chars", data.desc = desc2, 
				ints.as.nums = FALSE, ints.as.factors=TRUE, chars.as.factors = TRUE)
