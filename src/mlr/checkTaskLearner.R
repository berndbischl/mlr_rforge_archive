...
checkTaskLearner <- function(lt, learner) {
	...
	if (td["has.missing"] && !ld@missings) {
		stop("Data set has missing values, but", wl@desc@id, "does not support that!")
	}
	if (td@n.feat["numerics"] > 0 && !ld@feat["numerics"]) {
		stop("Data set has numeric inputs, but", wl@desc@id, "does not support that!")
	}
	if (td@n.feat["factors"] > 0 && !ld@feat["factors"]) {
		stop("Data set has factor inputs, but", wl@desc@id, "does not support that!")
	}
