...
he now defined task contains basic information about the data,
...
LearnTask_ class <a and its subclasses ClassifTask_ and RegrTask_, if
...
## Get target values for all observations.
head(getTargets(rt))

## Inspect <a href="rdocs/TaskDesc.html">TaskDesc</a>.
rt@desc@target
@ 
...
.. |learn_task| image:: /_images/learn_task.png
     :align: middle
     :width: 40em
