...
mod <- train("classif.lda", task = task, subset = seq(1,150,2))
...
mod <- train("classif.rpart", task = task)
...
training set and calcuate the mean of squared errors and the mean of absolute 
...
mod <- train("regr.gbm", task, subset = train.set) 
...
It is possible to implement your own performance measure by the function 
makeMeasure_. See ??? for details.
...
.. _makeMeasure: ../_static/rdocs/mlr/makeMeasure.html
.. _Measure: ../_static/rdocs/mlr/Measure.html
