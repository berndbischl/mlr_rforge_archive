...
task <- makeClassifTask(data = BreastCancer, target = "Class", exclude = "Id")
@ 
...
<<>>=
task <- makeClassifTask(data = BreastCancer, target = "Class", 
                      exclude = "Id", positive = "malignant")
@ 

The makeClassifTask_ help page list several other arguments that can be 
...
The rest works analogous to the classification example.
