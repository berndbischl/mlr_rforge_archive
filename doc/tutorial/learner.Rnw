...
learners_ documentation page or go to IntegratedLearners_.
...
* You can set an id which is quite useful if you conduct larger experiments and need to access results of a certain learning method.
* For classification tasks you have to create a Learner_ object if you want to control the type of prediction_ which is returned. Per default you get the class labels when making a prediction_. In order to get e.g. posterior probabilities you have to specifiy this when creating the learner.
...
<<>>=
library("mlr")
lrn <- makeLearner(class = "classif.rpart")
lrn
@                 
...
and properties of the data the learning method can deal with, for example if 
only two- or multi-class problems are supported and if it can cope with case 
weights or missing values.
...
<<>>=
lrn <- makeLearner(class = "classif.rpart", minsplit = 7, cp = 0.03, id = "myrpart")
lrn
@
...
<<>>=
lrn <- makeLearner(class = "classif.rpart", predict.type = "prob")
lrn
@

In case of Support Vector Machines it is also possible to access the class
membership scores.

<<>>=
lrn <- makeLearner(class = "classif.svm", predict.type = "scores")
lrn
@
...
Create an rpart Learner_ for regression. 

<<>>=
lrn <- makeLearner(class = "regr.rpart")
lrn
@                 
...
<<>>=
lrn <- makeLearner("regr.gbm", n.trees = 500, distribution = "laplace", interaction.depth = 3, id = "mygbm")
lrn
@
