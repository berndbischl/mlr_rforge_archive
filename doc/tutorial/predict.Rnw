...
#One to train the classifier and the second one as a test set.
...
we can specifiy a positive class when generating the task. Moreover, we can set the
...
.. _getProb: ../_static/mlr/getProb.html
