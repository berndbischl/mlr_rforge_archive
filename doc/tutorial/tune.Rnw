...
strategy, we add a discrete parameter for the ``C`` and ``sigma`` parameter of
the SVM to the parameter set.

<<>>=
ps <- makeParameterSet(makeDiscreteParameter("C", 2^(-1:1)),
...
<<>>=
tune("classif.ksvm", task=task, resampling=rdesc, par.set=ps, control=ctrl)
@ 
...
ps <- makeParameterSet(makeDiscreteParameter("k", 1:7))
...
tune("regr.kknn", task=task, resampling=rdesc, par.set=ps, control=ctrl, measures=mse)
@ 
