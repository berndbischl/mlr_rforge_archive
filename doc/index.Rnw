...
The framework currently focusses on supervised methods like classification 
...
* **mlrTune**: Tune hyper-parameters of a learner with different optimization algorithms. 
  Multi-criteria optimization is possible, too. The package also contains a model-based sequential 
